...
def Camera2Run ():
    ...
    try:
        ...
        return
    # setting up GPIO
    GPIOpin = configDict.get ('GPIOpin')
    if GPIOpin != 0:
        GPIO.setmode (GPIO.BCM)
        GPIO.setup(configDict.get ('GPIOpin'), GPIO.IN)
    recTimeOutMsec = configDict.get ('maxRecSecs') * 1e03
    isCapturing=False
    while True:
        if isCapturing == True:
            ...
            if GPIOpin != 0:
                GPIO.wait_for_edge (GPIOpin, GPIO.FALLING, timeout= int (recTimeOutMsec))
        else:
        ...
        try:
            ...
            if addStr == configDict.get ('UDP_Sender'):
                if ((isCapturing == False) and (dataStr != 'Stop')):
                    if GPIOpin != 0:
                        GPIO.wait_for_edge (GPIOpin, GPIO.RISING);print ('Edge arrived')
